...
const {
  UserRoleTypes,
  UserStatusTypes,
} = require('../constants/add.user.props.js');
...
describe('Create/delete user e2e test', () => {
  it('should login with valid credentials, add and remove user', async () => {
    ...
    console.log(
      `Addining user with username: ${addUserProps.userName}, employee name: ${addUserProps.employeeName}, and password ${addUserProps.password}`
    );
    ...
    const isDeletedUser = await AdminTabScreen.checkThatUserWasDeleted(
      addUserProps.userName
    );
